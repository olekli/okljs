...
const { make_result, get_ok, get_err, ok, err, match_result, is_result, make_ok, make_err, useJestResultMatcher, map_result, unwrap_result, unzip_result } = require("./result.js");
...
describe("unwrap_result", () => {
  it("returns value if ok", () => {
    let x = make_ok("some value");
    expect(unwrap_result(x)).toEqual("some value");
  });

  it("throws if err", () => {
    let x = make_err("some error");
    expect(() => unwrap_result(x)).toThrow("some error");
  });
});

describe("unzip_result", () => {
  it("returns ok when all entries are ok", () => {
    let x = [
        make_ok("a"),
        make_ok("b"),
  ...
  it("returns err when one entry is err", () => {
    let x = [
        make_ok("a"),
        make_err("b"),
        make_ok("c")
