...
const { register_type, make_type_static, match_type, assert_type } = require('./types.js');
...
test('making unregistered type fails', () => {
  expect(() => make_type_static({ value: 'some string' }, 'UnregisteredType')).toThrow();
});
...
test.each([
  ...
  make_type_static(obj, correct_type);
  expect(() => assert_type(obj, wrong_type)).toThrow();
});

test('TreeNode', () => {
  let tree = make_type_static(
    {
      left: {
        left: { value: '12' },
        value: '5',
...
test('Polymorphism', () => {
  let derived1 = make_type_static(
    { value: 'derived1' },
    [ 'Base', 'Derived1' ]
  );
  let derived2 = make_type_static(
    { value: 'derived2' },
    [ 'Base', 'Derived2' ]
  );
